...
import { diagnose as runDiagnose, KubewhyError, ReportSummary } from '../utils/cli';
    ...
    return vscode.window.withProgress(
        {
            ...
            title: `Diagnosing ${pod}…`,
            cancellable: false,
        },
        async () => {
            try {
                const report = await runDiagnose(pod!, namespace!);
                panel.show(report);
                return report;
            } catch (err) {
                const msg = err instanceof KubewhyError ? err.message : String(err);
                vscode.window.showErrorMessage(`Kubewhy diagnose failed: ${msg}`);
                return undefined;
            }
        },
    );
